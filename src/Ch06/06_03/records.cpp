...
int StudentRecords::get_students_amount() const {
    return students.size();
}

void StudentRecords::report_cards(std::string reportFilename){
    std::ofstream reportFile;
    reportFile.open(reportFilename);
    if(reportFile.fail())
        std::cout<<"Couldn't open report cards file!"<<std::endl;
    else {
        for(int i=1;i<=get_students_amount();i++){
            float points = 0.0f, credits = 0.0f;
            reportFile << "Report Card for " << get_student_name(i) << std::endl;
            for(Grade& grd : grades){
                if(grd.get_student_id() == i) {
                    reportFile << get_course_name(grd.get_course_id())<<": "<<grd.get_grade()<<std::endl;
                    unsigned char current_credits = get_course_credits(grd.get_course_id());
                    credits += current_credits;
                    points += get_num_grade(grd.get_grade()) * current_credits;
                }
            }
            reportFile << "GPA: "<<(points / credits) << std::endl;
            reportFile << std::endl;
        }
    }
}